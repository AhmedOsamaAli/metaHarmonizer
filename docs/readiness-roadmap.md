...
| P1 | ✅ Define and approve public-beta service objectives | Project-owner approval recorded in `docs/service-level-objectives.md` on 2026-08-15 with review due 2026-11-15: 99% monthly availability; dashboard p95 <=1 s/p99 <=2 s; zero controlled-fixture errors; warm harmonization p95 <=60 s; queue/storage safeguards. Institutional RPO/RTO remain separately pending. |
...
| P1 | ✅ Instrument rolling active-user demand without conflating accounts/sessions | Redis tracks distinct authenticated users seen in five minutes; operations reports separate rolling active users from registered accounts and alert at 40/50 measured planning thresholds |
| P2 | Add remaining capacity signals | Export oldest queued-job age, API/worker CPU, aggregate DB pool waits/connections, and external bandwidth into retained reports | Mixed read/real-ML report proves scale decisions from observed bottlenecks rather than a single user count |
